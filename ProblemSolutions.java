...
public class ProblemSolutions {
    ...
    public static void selectionSort(int[] values, boolean ascending ) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            // Swap the found value with the current position
            int temp = values[i];
    ...
    private void mergeDivisbleByKFirst(int arr[], int k, int left, int mid, int right){
        ...
        while (i <= mid && j <= right) {
            boolean DivisibleByI = arr[i] % k == 0;
            boolean DivisibleByJ = arr[j] % k == 0;

            if (DivisibleByI) {
                temp[index++] = arr[i++];
            } else if (DivisibleByJ) {
                temp[index++] = arr[j++];
            } else {
                // Normal merge sort logic for non-divisible elements
                if (arr[i] <= arr[j]) {
                    temp[index++] = arr[i++];
                } else {
                    temp[index++] = arr[j++];
    ...
    public static boolean asteroidsDestroyed(int mass, int[] asteroids) {
        ...
        long planet = mass; // Use long to handle potential overflow
